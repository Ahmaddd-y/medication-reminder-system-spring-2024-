...
# --- PROLOG SETUP ---
prolog = Prolog()
prolog.consult("interactions.pl")
...
# --- C++ REMINDER ---
def run_cpp_reminder(medication, time):
    try:
        subprocess.run(["./reminder", medication, time])
    except Exception as e:
        print("C++ Reminder Error:", e)
...
def check_interaction_func():
    ...
    drug_list = [d.strip().lower().replace(" ", "_") for d in meds.split(",") if d.strip()]
    if len(drug_list) < 2:
    ...
    # First Prolog check
    query1 = list(prolog.query(f"interacts({drug_list[0]}, {drug_list[1]})"))
    query2 = list(prolog.query(f"interacts({drug_list[1]}, {drug_list[0]})"))
    if query1 or query2:
        messagebox.showerror("Interaction", f"⚠️ Warning: {drug_list[0]} and {drug_list[1]} may interact!")
        return

    # If Prolog safe, ask AI
...
def ask_ai_func():
    ...
    if not query:
        ...
        return
    prompt = f"You are a helpful medical assistant: {query}"
    try:
        result = check_interaction([prompt], model_type=model_type.get())
        response = result.content if hasattr(result, "content") else str(result)
...
def save_reminder():
    ...
    choice = messagebox.askyesno("Reminder", f"Do you want to set an instant reminder for {med}?")
    if choice:
        run_cpp_reminder(med, t)
